	...
	<?php 
		...
		if($num_rows>0){
			...
			while($row=mysqli_fetch_assoc($result)){
				$ISBN=$row['ISBN'] ;
				echo "<tr><td>". $row['title'] . "</td><td>". $row['pubName'] . "</td><td>" .$row['numPages'] ."</td><td>" .$row['ISBN'] ."</td> <td>" .$row['copiesNum'] ."</td><td><form action='findBook.php' method='post'><button><img src='css/search.png'  /></button><input type='hidden' name='ISBN' value='$ISBN'/></form></td></tr>";
			}
